...
<body data-id="{{Auth::id()}}" data-they-id="{{$theyId ?? ''}}">
<div id="app"></div>
<div class="container">
    <div  class="mt-4 py-2">
        <div id="messageBox" class="row py-2 m-0">
            @if(count($message) > 0 && !empty($user))
                @foreach($message as $mess)
                    @if($mess->sender == $user->id)
                        <div class="col-12">
                            <img src="{{$user->avatar}}" class="float-start img-fluid rounded-circle" alt="avatar" style="width: 60px; height: 60px">
                            <div class="col-8 col-md-7 col-lg-6 alert alert-primary float-start mx-2" role="alert">{{$mess->content}}</div>
                        </div>
                    @else
                        <div class="col-12"><div class="col-8 col-md-7 col-lg-6 alert alert-success float-end" role="alert">{{$mess->content}}</div></div>
                    @endif
                @endforeach
            @endif
        </div>
    ...
    <script>
        ...
        var theyId = $('body').attr('data-they-id');
